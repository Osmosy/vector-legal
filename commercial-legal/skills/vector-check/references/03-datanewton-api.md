...
## Ограничения демо-доступа (важно)

Демо-ключ даёт только базовые справочные данные. Для полноценной работы нужен **платный тариф**:

| Что | В демо | На платном |
|---|---|---|
| Карточка ЕГРЮЛ/ЕГРИП (counterparty-basic) | ✅ работает | ✅ |
| Арбитраж, ФССП, банкротство, госконтракты | ✅ работают (единицы лимита) | ✅ |
| **Скоринг** (scoring) | ⚠️ заглушка (HIGH/100 всем) | ✅ реальный |
| **Финансы/бухотчётность** (finance, taxInfo, paidTaxes) | ❌ пусто | ✅ |
| **Полные риски** (risks: has_risks_access) | ❌ false | ✅ |
| **Граф связей** (links: has_related_companies_access) | ❌ false | ✅ |
| **MCP-сервер** (роль API_MCP_FULL) | ❌ 403 | ✅ отдельный тариф через поддержку |

Лимит демо-ключа (~999 ед. на 2026-09-03) ≈ 150–300 полных проверок.

**Как перейти на платный:** self-serve покупки MCP/API-тарифа на сайте нет — письмо в поддержку info@datanewton.ru (email аккаунта + use-case + просьба включить роль API_MCP_FULL / расширенный API).